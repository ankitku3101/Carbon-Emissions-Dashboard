...
# Save to CSV
csv_filename = "./dataset/coal_power_plants.csv"
with open(csv_filename, mode="w", newline="") as file:
    writer = csv.writer(file)
    writer.writerow(["_id", "coaluses", "plf", "production", "totalemission", "createdAt", "updatedAt"])
    
    for entry in data:
        writer.writerow([
            entry["_id"],
            entry["coaluses"],  # MongoDB can parse JSON directly
            entry["plf"],
            entry["production"],
            entry["totalemission"],
            entry["createdAt"],
